...
export function ProductsModule() {
  ...
  function toggleTag(id: string) {
    setItems((prev) => prev.map((p) => p.id === id ? { ...p, tag: p.tag === "BEST" ? null : "BEST" } : p));
  }
      ...
      <div className="adm-content">
        <div className="adm-topbar">
          <div>
            ...
            <div className="adm-sub">{items.length} produits · {items.filter((p) => p.stock === 0).length} épuisés</div>
          </div>
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td>
                      <span className="adm-cat-pill">{catName(p.cat)}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      {p.tag ? (
                        <span className="adm-badge" style={{ color: "#B8902B", background: "rgba(212,175,55,.12)", cursor: "pointer" }} onClick={() => toggleTag(p.id)}>{p.tag}</span>
                      ) : (
                        <button className="adm-link" style={{ fontSize: 12 }} onClick={() => toggleTag(p.id)}>+ Best</button>
                      )}
                    </td>
                    <td>
